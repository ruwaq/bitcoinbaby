...
describe("minedAmountBro (canonical BRO reward)", () => {
  it("D16 @ period 0 = 256 BABTC (25_600_000_000 base units)", () => {
    ...
    expect(minedAmountBro(16, BRO_START_TIME)).toBe(25_600_000_000n);
  });

  it("halves after one halving period", () => {
    const period0 = minedAmountBro(16, BRO_START_TIME);
    const period1 = minedAmountBro(
      16,
      BRO_START_TIME + BRO_HALVING_PERIOD_SECONDS,
    );
  ...
  it("quarters after two halving periods", () => {
    ...
    const period2 = minedAmountBro(
      16,
      BRO_START_TIME + 2 * BRO_HALVING_PERIOD_SECONDS,
    );
  ...
  it("clz=0 yields 0 reward", () => {
    expect(minedAmountBro(0, BRO_START_TIME)).toBe(0n);
  });

  it("D32 @ period 0 = 1e8 * 1024 (clz^2 = 1024)", () => {
    expect(minedAmountBro(32, BRO_START_TIME)).toBe(
  ...
  it("scales quadratically with clz at the same period", () => {
    ...
    const r16 = minedAmountBro(16, BRO_START_TIME);
    const r20 = minedAmountBro(20, BRO_START_TIME);
  ...
  it("clamps blockTime below START_TIME to START_TIME (no negative halvings)", () => {
    const before = minedAmountBro(16, BRO_START_TIME - 1_000_000);
    const atGenesis = minedAmountBro(16, BRO_START_TIME);
    expect(before).toBe(atGenesis);
    expect(before).toBe(25_600_000_000n);
  });

  it("clamps negative clz to 0", () => {
    expect(minedAmountBro(-5, BRO_START_TIME)).toBe(0n);
  });
  ...
  it("never throws on extreme inputs (overflow safety)", () => {
    ...
    const farFuture = BRO_START_TIME + 1000 * BRO_HALVING_PERIOD_SECONDS;
  ...
  it("saturates the halving factor at the cap (rewards stop halving past 63 periods)", () => {
    ...
    const atCap = minedAmountBro(
      16,
      BRO_START_TIME + 63 * BRO_HALVING_PERIOD_SECONDS,
    );
    const pastCap = minedAmountBro(
      16,
      BRO_START_TIME + 100 * BRO_HALVING_PERIOD_SECONDS,
    );
...
describe("merkle.ts calculateMiningReward delegates to canonical", () => {
  ...
  // BRO_START_TIME must reproduce minedAmountBro exactly.
  it("matches minedAmountBro when startTime == BRO_START_TIME", () => {
    ...
    expect(viaMerkle).toBe(minedAmountBro(16, BRO_START_TIME));
...
describe("token.ts calculateMiningRewardBro (canonical variant)", () => {
  it("uses canonical BRO total (NOT the legacy /100 value)", () => {
    ...
    const canonical = calculateMiningRewardBro(16, BRO_START_TIME);
  ...
  it("clamps difficulty to the SPARK range (D16..D32)", () => {
    const below = calculateMiningRewardBro(5, BRO_START_TIME);
    const above = calculateMiningRewardBro(99, BRO_START_TIME);
  ...
  it("applies the 90/5/5 distribution and sums to total", () => {
    const r = calculateMiningRewardBro(20, BRO_START_TIME);
  ...
  it("halves the total after one period", () => {
    const p0 = calculateMiningRewardBro(16, BRO_START_TIME).total;
